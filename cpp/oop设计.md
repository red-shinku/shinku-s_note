...
位于继承体系中的类也需要控制当其对象执行一系列操作时发生什么行为，这些操作包括创建、拷贝、移动、赋值和销毁。

---

### 4.1 虚析构函数

现在应该清楚为什么基类通常需要一个虚析构函数了。在 第3节 以解释。只要基类的析构函数是虚函数，就能确保我们delete基类指针时能运行正确的版本（根据其动态类型）。

#### 虚析构函数将阻止合成移动操作

如果在类中定义了虚析构函数，编译器将不再为这个类合成默认的移动操作（移动构造函数和移动赋值运算符）。这是因为如果存在基类，通常意味着存在多态，如果需要移动操作的话，你必须自己写一个安全的版本。（实际上非虚的析构函数也会阻止合成移动操作，但是在继承中更敏感）。
